...
class AISafetyPredictor:
    ...
    def _generate_training_data(self):
        ...
        # Features: hour, day_of_week, weather_score, distance_to_police, population_density
        hours = np.random.randint(0, 24, n_samples)
        days = np.random.randint(0, 7, n_samples)
        weather = np.random.uniform(0, 100, n_samples)  # 0=bad, 100=good
        police_dist = np.random.uniform(0, 5000, n_samples)  # meters
        population = np.random.uniform(0, 10000, n_samples)  # people per km²
        
        # Crime risk logic (higher at night, bad weather, far from police)
        crime_risk = (
            50 + 
            (24 - hours) * 2 +  # Higher at night
            (100 - weather) * 0.3 +  # Higher in bad weather
            police_dist * 0.01 +  # Higher far from police
            np.random.normal(0, 10, n_samples)  # Random noise
        )
        crime_risk = np.clip(crime_risk, 0, 100)
        
        # Crowd density logic (higher during day, good weather, commercial areas)
        crowd_density = (
            30 +
            np.where(6 <= hours, hours * 3, 0) +  # Higher during day
            np.where(hours <= 18, (18 - hours) * 2, 0) +
            weather * 0.4 +  # Higher in good weather
            population * 0.003 +
    ...
    def predict_crime_pattern(self, lat, lng, hour=None, day_of_week=None):
        ...
        # Estimate population density (mock)
    ...
    def _estimate_population_density(self, lat, lng):
        ...
        # Commercial areas have higher density
        commercial_zones = [
            (18.5404, 73.8767), (18.5604, 73.7767)  # Malls
        ]
        
        base_density = 2000
        for c_lat, c_lng in commercial_zones:
            distance = self._haversine_distance(lat, lng, c_lat, c_lng)
            if distance < 1000:  # Within 1km of commercial zone
